...
export default function MobileProductCarouselSection({
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter products by product type and only include 'singles' products
    ...
    if (productType === 'singles') {
      if (category === 'sosis') {
        return (
          ...
          >
            Sosis siap makan dengan berbagai varian 
            rasa. Praktis bisa dimakan di mana pun & 
            kapan pun.
          </p>
        );
        ...
        return (
          <p
            className={`text-center w-full max-w-sm text-base ${poppins.className}`}
          >
            Bakso siap makan dengan berbagai varian 
  ...
  return (
    <section
      className="relative min-h-screen overflow-hidden max-h-[80vh] "
    ...
    >
      ...
      {/* Main Content */}
      <div
        className="relative z-10 min-h-screen flex flex-col items-center justify-center px-8 py-12 pt-28 max-h-[80vh]"
        id="produk"
      >
        ...
        <motion.div
          className="mb-6 text-white/80 w-full flex justify-center max-w-[260px]"
        ...
        <div className="flex-1 flex items-center justify-center relative w-full mb-6 -mt-24 ">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Back Image (Mockup/Thermopack) - if available */}
              {currentProduct?.secondImage && (
                <div
                  className="absolute"
                  style={{
                    transform:
                      'translateX(50px) translateY(10px) scale(0.5) rotate(10deg)',
                    zIndex: 1,
                    // filter: 'blur(2px)',
                    // opacity: 0.6,
                  }}
              ...
              {/* Front Image (Main Product) */}
              <div
                className="relative"
                style={{
                  zIndex: 2,
                  transform:
                    'translateX(-30px) translateY(0px) scale(0.8) rotate(-15deg)',
                }}
              >
                <Image
